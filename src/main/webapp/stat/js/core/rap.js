...
(function () {
     ...
     * update action desc
     */
    p.updateActionDesc = function (obj) {
        var action = this.getAction(obj.id);
        if (action !== null) {
            // action.name = obj.name;
            // action.requestType = obj.requestType;
...
(function () {
    ...
    ws.init = function (workspaceObj, urlObj, actionId) {
        ...
        b.dom.ready(function () {
            ...
            $("#inputAuthor,#inputVersion,#selectStatus").bind("blur",function(){
                _filter={};
                if($("#inputAuthor").val() != '')
                {
                    _filter.author=$("#inputAuthor").val();
                }
                if($("#inputVersion").val() != '')
                {
                    _filter.version=$("#inputVersion").val();
                }
                if($("#selectStatus").val() != '请选择')
                {
                    _filter.status=$("#selectStatus").val();
                }
                updateCurMTree();
            });
    ...
    ws.switchA = function (actionId, forceRefresh) {
        ...
        ws.initEditorView(action.description);
     ...
     * add or edit action
     */
    ws.doAddOrEditDesc = function () {
        if (!validate("formEditDescFloater")) return;
        if (b.g("editDescFloater-id").value === "") {
            this.doAddDesc();
        } else {
            this.doEditDesc();
        }
    ...
    ws.editA = function (actionId) {
        ...
        b.g("editAFloater-author").value = action.author ? action.author : '';
        b.g("editAFloater-version").value = action.version ? action.version : '';
        b.g("editAFloater-status").value = action.status ? action.status : '';
     ...
     * edit description
     */
    ws.editDesc = function (actionId) {
        if (!_isEditMode) return;
        var action = p.getAction(actionId);
        initEditAFloater();
        b.g("editDescFloater-id").value = action.id;
        // b.g("editDescFloater-name").value = action.name;
        // setSelectedValue("editDescFloater-type", action.requestType);
        var struct = getActionStruct(action);
        setSelectedValue("editDescFloater-struct", struct);
        // hide action struct in description textarea
        var desc = action.description;
        var code = '@type=array_map;';
        if (desc && desc.substring(0, 16) === code) {
        ...
        // b.g("editDescFloater-requestUrl").value = action.requestUrl;
        // b.g("editDescFloater-responseTemplate").value = action.responseTemplate;
        b.g("editDescFinitloater-description").value = desc;
        e.get("editDescFloater").setTitle("模型管理 - 编辑请求");
        _editor.previewing();
        ecFloater.show("editDescFloater");
    };
    ...
    ws.doEditA = function (actionId) {
        ...
        action.author=b.g("editAFloater-author").value;
        action.version=b.g("editAFloater-version").value;
        action.status=b.g("editAFloater-status").value;
        // action.description = b.g("editAFloater-description").value;
        action.description=_editor.getMarkdown();
     ...
     * submit action edit operation
     */
    ws.doEditDesc = function () {
        // loading action
        var action = {};
        action.id = b.g("editDescFloater-id").value - 0;
        // action.name = b.g("editAFloater-name").value;
        // action.requestType = getSelectedValue("editAFloater-type");
        // action.requestUrl = b.g("editAFloater-requestUrl").value;
        // action.responseTemplate = b.g("editAFloater-responseTemplate").value;
        // action.description = b.g("editDescFloater-description").value;
        action.description = _editor.getMarkdown();
        var struct = getSelectedValue("editDescFloater-struct");
        setActionStruct(action, struct);

        // update model
        p.updateActionDesc(action);
     ...
     * cancel action edit operation
     */
    ws.cancelEditDesc = function () {
        ecui.get("editDescFloater").hide();
    };
    ...
    ws.initEditorView = function (desc) {
        _editorView = editormd.markdownToHTML("editormd-view-"+_curModuleId, {
            markdown:desc ? desc : '',//+ "\r\n" + $("#append-test").text(),
        ...
        _editor = editormd("editormd", {
            width: "100%",
            height: 500,
            markdown: _editorView.getMarkdown() ? _editorView.getMarkdown() : '',
            // codeFold: true,
    ...
    /**
     * init edit description floater
     */
    function initEditDescFloater() {
        b.g("editDescFloater-id").value = "";
        b.g("editDescFloater-name").value = "";
        b.g("editDescFloater-requestUrl").value = "";
        b.g("editDescFloater-responseTemplate").value = "";
        // b.g("editAFloater-description").value = "";
        b.g("editDescFloater-pageId").value = "";
        initRadioList("editDescFloater-type");
        initRadioList("editDescFloater-struct");
    }
            ...
            for (var j = 0; j < actionListNum; j++) {
                ...
                if(doFilter(action))
                {
                    ...
                    str += "<div id=\"div-a-tree-node-" + action.id + "\">" + "<a href=\"#\" onclick=\"ws.switchA(" + action.id +
                        "); return false;\" ondblclick=\"ws.editA(" + action.id + "); return false;\" style=\"color:"+color+"\">" + util.escaper.escapeInH(action.name) + "</a>";
                    if (_isEditMode) {
                        str += "<a href=\"#\" class=\"edit-link\" onclick=\"ws.editA(" +
                            action.id + "); return false;\"  style=\"color:"+color+"\"><i class=\"glyphicon glyphicon-pencil\"></i><a href=\"#\" onclick=\"ws.removeA(" +
    ...
    {
        return (!_filter || ((!_filter.author || (action.author && action.author.indexOf(_filter.author) > -1)) &&
            (!_filter.version || (action.version && action.version.indexOf( _filter.version)> -1)) &&
            (!_filter.status || (action.status && action.status.indexOf(_filter.status)> -1))));
    }
    ...
    function getAInfoHtml(a) {
        ...
        if (a.author) {
            body += "<div class='item'><b>负&nbsp;&nbsp;责&nbsp;&nbsp;人 </b>" + a.author + "</div>";
        }
        if (a.version) {
            body += "<div class='item'><b>版&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;本 </b>" + a.version + "</div>";
        }
        if (a.status) {
            body += "<div class='item'><b>状&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;态 </b>" + a.status + "</div>";
        }
        if (a.requestUrl) {
            body += "<div class='item'><b>请求&nbsp;&nbsp;Url </b><font color='blue'> " + a.requestUrl + "</font></div>";
        }
        if (a.responseTemplate) {
            body += "<div class='item'><b>相关模板 </b><font color='red'>" + a.responseTemplate + "</font></div>";
        }

        var deschtml = "<div class='action-info' href='#' onclick='ws.editDesc("
            + a.id + "); return false;'><div class='item'><b>接口描述 </b>";
        deschtml += '<a class="div-a-desc-expander" href="#" onclick="this.innerHTML = this.innerHTML == \'显示\' ? \'隐藏\' : \'显示\';baidu.each(baidu.dom.query(\'.description-area\'), function(ele){baidu.dom.toggle(ele);}); return false;">显示</a></div>';
        deschtml += "<div class='item'>" + processEditor(a.description) + "</div></div>";


        if (!body) {
            body += "no info";
        }
        return head + body + foot + deschtml;
    }
    ...
    function processEditor(txt) {
        ...
        var arr = [],i = 0;
            arr[i++] = '<div class="description-area" id="editormd-view-'+_curModuleId+'\" style="display: none;">';
            arr[i++] = '<textarea id="description-textarea-preview" style="display:none;">';
            arr[i++] = txt;
            arr[i++] = '</textarea></div>';
            arr[i++] = '</textarea>';
