...
import com.taobao.rigel.rap.common.utils.DomainEquals;
import com.taobao.rigel.rap.common.utils.MailUtils;
import com.taobao.rigel.rap.common.utils.MapUtils;
import com.taobao.rigel.rap.mail.MailUtil;
    ...
    public String checkIn() throws Exception {
        ...
        for (Module module : project.getModuleList()) {
            for (Page page : module.getPageList()) {
                oldActionList.addAll(new ArrayList<Action>(page.getActionList()));
            }
        }
        ...
        Project newProject = gson.fromJson(project
                .toString(Project.TO_STRING_TYPE.TO_PARAMETER), Project.class);

        for (Module module : newProject.getModuleList()) {
            for (Page page : module.getPageList()) {
                newActionList.addAll(new ArrayList<Action>(page.getActionList()));
            }
        }


        //update items
        List<Map<String, Object>> changeList = GetChangeList(oldActionList, newActionList);

        logger.info("开始处理邮件通知");
        /******************发送邮件通知**********************/
        String templateName = "template_1.ftl";
        Map<String, Object> emailInfo = new HashMap<String, Object>();
//        map.put("name", "测试接口");
        emailInfo.put("updater", getCurUser().getName());
        emailInfo.put("updateTime", project.getUpdateTime().toGMTString());
        emailInfo.put("items", changeList);
...
//                    MailUtil.sendMailByTemplate(user.getEmail(), project.getName(), emailInfo, templateName);
                }
            }
        }
        ...
        if (notification.getUser().getId() != getCurUserId()) {
            getAccountMgr().addNotification(notification);
            if (changeList.size() > 0) {
//                MailUtil.sendMailByTemplate(getCurUser().getEmail(), project.getName(), emailInfo, templateName);
            }
        }
        projectMgr.updateCache(id);

        logger.info("结束处理邮件通知");
    ...
    private List<Map<String, Object>> GetChangeList(Set<Action> oldActionList, Set<Action> newActionList) {
        ...
        for (Action action1 : newActionList) {
            ...
            if (!flag) {
                item = new HashMap<>();
                item.put("projectID", new Integer(id).toString());
                item.put("itemID", action1.getId());
                item.put("itemName", action1.getName());
                item.put("modifyType", "新增");
                changeList.add(item);
            }
        }
